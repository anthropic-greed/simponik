...
export default function LoginForm() {
  ...
  return (
    <div
      ...
      style={{
        backgroundImage: "url('/kantor-gerbang.jpg')",
    ...
    >
      ...
      >
        <div className="flex flex-col items-center mb-8">
          <img src="/logo-simponik-mark.png" alt="SIMPONIK"
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <input type="email" placeholder="Email" value={email}
            ...
            className="w-full rounded-xl px-4 py-3 text-sm text-white placeholder-white/40 outline-none"
          ...
          <input type="password" placeholder="Kata sandi" value={password}
            ...
            onBlur={e => (e.currentTarget.style.border = '1px solid rgba(255,255,255,0.15)')} />
          {error && (
            <p className="text-red-400 text-xs text-center bg-red-500/10 rounded-lg px-3 py-2">{error}</p>
          )}
          <button type="submit" disabled={loading}
            className="w-full rounded-xl py-3 text-sm font-bold text-white disabled:opacity-60"
            style={{ background: 'linear-gradient(90deg, #1d4ed8, #2563eb)', boxShadow: '0 4px 20px rgba(37,99,235,0.4)' }}>
            {loading ? 'Memuat...' : 'Masuk'}
          </button>
        </form>
        <p className="text-center text-white/30 text-xs mt-6">
          2025 SIMPONIK - Kantor Imigrasi Kelas II TPI Tanjung Balai Karimun
        </p>
      </div>
    </div>
  )
